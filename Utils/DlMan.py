...
class MapDownloadManager(object):
    ...
    def _HttpLoadFile(self, url, filename):
        ...
        print("HttpLoadFile open file {}".format(filename))
         with open(filename,'wb') as f:
            print("HttpLoadFile write {} bytes".format(len(html)))
            f.write(html)
    ...
    def CheckExternelUtils(self):
        ...
        urlImageMagick      = 'ftp://ftp.imagemagick.org/pub/ImageMagick/binaries/ImageMagick-7.0.7-11-portable-Q16-x86.zip'
        
        ensure_dir(downloadPath)
        
        if(os.path.isfile(FileNameImageMagick)==False):
            print("start download {}".format(urlImageMagick))
            filename = self._HttpLoadFile(urlImageMagick, FileNameImageMagick)
            
        if(os.path.isdir(PathNameImageMagick)==False):
        ...
        if(os.path.isfile(FileName_imgkap)==False):
    ...
    def MergeTiles(self, ti ):
        tempfilename = "{}/seamapmerged/{}/{}/map.png".format(self.WorkingPath, ti.name, ti.zoom)
    ...
    def GenKapFile(self, ti ):            
        tempfilename = "{}/seamapmerged/{}/{}/map.png".format(self.WorkingPath, ti.name, ti.zoom)
        kapdirname   = "{}/kap/{}/{}".format(self.WorkingPath, ti.name, ti.zoom) 
        kapfilename  = "{}/map.kap".format(kapdirname) 
        
        if not os.path.exists(kapdirname):
            os.makedirs(kapdirname)
        
        if(os.path.isfile(kapfilename)==False): 
